import { f as R, L as Oe, j as Y, A as Rt, av as yt, z as Ce, Z as Zt, y as Pe } from "./TriangleExclamation.D9eaO6CL.js";
const Se = ["top", "right", "bottom", "left"], U = Math.min, W = Math.max, ft = Math.round, lt = Math.floor, j = (t) => ({
  x: t,
  y: t
}), Ee = {
  left: "right",
...
async function it(t, e) {
  ...
  const {
    ...
    altBoundary: h = !1,
    padding: d = 0
  } = q(e, t), p = Ut(d), w = c[h ? u === "floating" ? "reference" : "floating" : u], g = dt(await r.getClippingRect({
...
const Ne = (t) => ({
  ...
  async fn(e) {
    ...
    const L = y / 2 - v / 2, H = C / 2 - m[p] / 2 - 1, S = U(u[g], H), F = U(u[x], H), N = S, P = C - m[p] - F, k = C / 2 - m[p] / 2 + L, D = vt(N, k, P), T = !l.arrow && et(i) != null && k !== D && r.reference[p] / 2 - (k < N ? S : F) - m[p] / 2 < 0, M = T ? k < N ? k - N : k - P : 0;
    return {
      [d]: h[d] + M,
      data: {
        [d]: D,
  ...
  return t === void 0 && (t = {}), {
    ...
    async fn(e) {
      ...
      if (S = [...S, {
        ...
        var F, N;
        const D = (((F = r.flip) == null ? void 0 : F.index) || 0) + 1, T = C[D];
        if (T) {
          var P;
...
const _e = function(t) {
  ...
  return t === void 0 && (t = {}), {
    ...
    async fn(e) {
      ...
      const x = m - f.top - f.bottom, b = p - f.left - f.right, y = U(m - f[w], x), v = U(p - f[g], b), O = !e.middlewareData.shift;
      let C = y, L = v;
      if ((n = e.middlewareData.shift) != null && n.enabled.x && (L = b), (o = e.middlewareData.shift) != null && o.enabled.y && (C = x), O && !h) {
        const S = W(f.left, 0), F = W(f.right, 0), N = W(f.top, 0), P = W(f.bottom, 0);
        d ? L = p - 2 * (S !== 0 || F !== 0 ? S + F : W(f.left, f.right)) : C = m - 2 * (N !== 0 || P !== 0 ? N + P : W(f.top, f.bottom));
      }
      await l({
        ...e,
        availableWidth: L,
...
function Qe(t, e) {
  ...
  for (; _(s) && !tt(s); ) {
    const c = z(s), l = St(s);
    !l && c.position === "fixed" && (i = null), (r ? !l && !i : !l && c.position === "static" && !!i && ["absolute", "fixed"].includes(i.position) || st(s) && !l && ee(t, s)) ? o = o.filter((f) => f !== s) : i = c, s = K(s);
  }
  return e.set(t, o), o;
}
function tn(t) {
  ...
  const s = [...n === "clippingAncestors" ? mt(e) ? [] : Qe(e, this._c) : [].concat(n), o], c = s[0], l = s.reduce((a, f) => {
    const u = It(e, f, i);
    return a.top = W(u.top, a.top), a.right = U(u.right, a.right), a.bottom = U(u.bottom, a.bottom), a.left = W(u.left, a.left), a;
  }, It(e, c, i));
  return {
...
function gn(t) {
  ...
  const [p, m] = R.useState(null), [w, g] = R.useState(null), x = R.useCallback((A) => {
  ...
  }, []), y = r || p, v = s || w, O = R.useRef(null), C = R.useRef(null), L = R.useRef(f), H = l != null, S = xt(l), F = xt(i), N = xt(a), P = R.useCallback(() => {
    ...
    F.current && (A.platform = F.current), mn(O.current, C.current, A).then((E) => {
      ...
      k.current && !pt(L.current, $) && (L.current = $, Oe.flushSync(() => {
        u($);
      }));
    });
  }, [h, e, n, F, N]);
  at(() => {
    a === !1 && L.current.isPositioned && (L.current.isPositioned = !1, u((A) => ({
      ...A,
      isPositioned: !1
    })));
  }, [a]);
...
fe.displayName = ae;
var Mt = "PopperContent", [Ln, Dn] = se(Mt), ue = R.forwardRef(
  (t, e) => {
    ...
    const {
      ...
      onPlaced: p,
      ...m
    } = t, w = ce(Mt, n), [g, x] = R.useState(null), b = Zt(e, (ot) => x(ot)), [y, v] = R.useState(null), O = Sn(y), C = (O == null ? void 0 : O.width) ?? 0, L = (O == null ? void 0 : O.height) ?? 0, H = o + (r !== "center" ? "-" + r : ""), S = typeof f == "number" ? f : { top: 0, right: 0, bottom: 0, left: 0, ...f }, F = Array.isArray(a) ? a : [a], N = F.length > 0, P = {
      padding: S,
      boundary: F.filter(Mn),
      // with `strategy: 'fixed'`, this is the only way to get it to respect boundaries
      altBoundary: N
    }, { refs: k, floatingStyles: D, placement: T, isPositioned: M, middlewareData: A } = gn({
      ...
      middleware: [
        xn({ mainAxis: i + L, alignmentAxis: s }),
        l && yn({
          mainAxis: !0,
          crossAxis: !1,
...
var de = "PopperArrow", Tn = {
  ...
  const { __scopePopper: o, ...i } = e, r = Dn(de, o), s = Tn[r.placedSide];
  return (
