...
import com.crunch42.openapi.snippets.SnippetLayout;
...
import static com.crunch42.openapi.utils.OpenAPIUtils.getFirstNotEmptyPanel;
import static com.crunch42.openapi.utils.OpenAPIUtils.getPadding;
...
public class AddSnippetAction extends Action {
  ...
  public boolean isEnabled() {
    ...
    if (o.isPanel() && o.is(GENERAL)) {
      ...
      Map<String, DefaultMutableTreeNode> pointerToNodesMap =
              dataService.getParserData(file.getFullPath().toPortableString()).getPointerToNodesMap();
  ...
  public void run() {
    ...
    OpenApiTreeNode o = (OpenApiTreeNode) node.getUserObject();
    IDataService dataService = (IDataService) PlatformUI.getWorkbench().getService(IDataService.class);
    ParserData parserData = dataService.getParserData(file.getFullPath().toPortableString());
    if (!parserData.isValid()) {
        return;
    }

    OpenApiFileType fileType = dataService.getFileProperty(file.getFullPath().toPortableString()).getType();
    ...
    final SnippetLayout layout = new SnippetLayout(padding, innerPadding, false,
            o.getKey(), true, null, parserData.isCrLfEol());
