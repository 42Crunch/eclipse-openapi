...
import static com.crunch42.openapi.utils.OpenAPIUtils.getMap;
import static com.crunch42.openapi.utils.OpenAPIUtils.getValue;
...
public class Audit {
    ...
    private List<Issue> transformIssues(OpenAPIBundler bundler, Map<String, Object> context,
                                        List<String> pointers, int defaultCriticality) {
        ...
        for (Map.Entry<String, Object> entry : issues.entrySet()) {
            ...
            for (Object subIssueItem : (JSONArray) issue.get("issues")) {
                ...
                String description = (String) (subIssue.containsKey("specificDescription") ?
                        getValue(subIssue, "specificDescription") : getValue(issue, "description"));

                float score = subIssue.containsKey("score") ?
                        Math.abs(((Number) getValue(subIssue, "score")).floatValue()) : 0;

                int criticality = issue.containsKey("criticality") ?
                        (Integer) getValue(issue, "criticality") : defaultCriticality;
