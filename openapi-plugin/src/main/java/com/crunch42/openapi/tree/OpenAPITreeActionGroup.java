...
import com.crunch42.openapi.actions.AddSnippetAction;
import com.crunch42.openapi.services.IDataService;
import com.crunch42.openapi.services.ISnippetService;
import com.crunch42.openapi.snippets.Snippet;
import com.crunch42.openapi.snippets.SnippetIDs;
...
public class OpenAPITreeActionGroup extends CompositeActionGroup {
    ...
    public void fillContextMenu(IMenuManager menu) {
        ...
        if (!dataService.getParserData(selectedFile.getFullPath().toPortableString()).isValid()) {
            return;
        }

        OpenApiFileType type = dataService.getFileProperty(selectedFile.getFullPath().toPortableString()).getType();
        boolean isJson = (type == OpenApiFileType.Json);
        OpenApiVersion version = dataService.getFileProperty(selectedFile.getFullPath().toPortableString()).getVersion();
        ...
        if (node.isPanel()) {
            if (node.is(OpenApiPanelKeys.GENERAL) && (version == OpenApiVersion.V3)) {
                ...
                snippetIds.add(isJson ? SnippetIDs.COMPONENTS_SECURITY_API_KEY : SnippetIDs.COMPONENTS_SECURITY_API_KEY_YAML);
                snippetIds.add(isJson ? SnippetIDs.COMPONENTS_SECURITY_BASIC : SnippetIDs.COMPONENTS_SECURITY_BASIC_YAML);
                snippetIds.add(isJson ? SnippetIDs.COMPONENTS_SECURITY_JWT : SnippetIDs.COMPONENTS_SECURITY_JWT_YAML);
                snippetIds.add(isJson ? SnippetIDs.COMPONENTS_SECURITY_OAUTH2_IMPLICIT : SnippetIDs.COMPONENTS_SECURITY_OAUTH2_IMPLICIT_YAML);
            }
        ...
        if (snippetIds.size() == 1 &&
                (SnippetIDs.OPERATION.equals(snippetIds.get(0)) ||
