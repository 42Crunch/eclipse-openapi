...
public class JCEFGqlScanConfPanel extends WebFileEditor
    ...
    public JCEFGqlScanConfPanel(@NotNull Project project, @NotNull Composite parent, @NotNull WebVirtualFile file) {
    ...
    protected @NotNull BrowserFunction getBrowserFunction(@NotNull Browser browser, @NotNull String name) {
        return new JCEFGqlScanConfFunction(project, myId, cache, browser, name);
    }

    @Override
    public void showScanConfOperation(@NotNull String webAppId,
        ...
        cache.put(GQL_SCAN_CONF_PATH, payload.getScanConfPath());
        new ShowScanConfOperation(payload).send(getCefBrowser());
    }

    @Override
    public void loadUpdatedScanConf(@NotNull String webAppId, @NotNull ScanConfWithOas payload) {
        if (!Objects.equals(webAppId, myId)) {
            return;
        }
