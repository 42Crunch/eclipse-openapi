...
public class Location {
	...
	public Location toYAMLLocationForCrLfEOL() {
		// YAML parsing library counts \r symbols in offsets, but also in lines that is
		// a bug
	...
	public Location toJSONLocationForCrLfEOL() {
		// JSON parsing library ignores \r symbols, but eclipse does consider it in
		// offset logic
		// So for \r\n EOL files we need to increase (restore) the offsets to the amount
		// of missing \r symbols = lines
	...
	public Location applyShift(int shift) {
		// We replace the key to show in the tree, but it is mapped to the same
		// position, so do not change both offsets
		return new Location(line, column + shift, startOffset, endOffset);
	}

	public TextRange getTextRange() {
		return new TextRange((int) getStartOffset(), (int) getEndOffset());
	}
}
