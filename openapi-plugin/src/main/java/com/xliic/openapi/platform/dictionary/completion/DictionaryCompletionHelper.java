...
public class DictionaryCompletionHelper {
    ...
    public static void updateResultSet(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet resultSet) {
        ...
        for (DictionaryElement item : ddService.getAllFormats()) {
            LookupElementBuilder builder = LookupElementBuilder.create(item.getElement()).
                    withPresentableText(item.getPresentableText()).
                    withIcon(OpenApiIcons.Dictionary).
                    withTypeText(item.getWithTypeText()).
                    withFileType(fileType).
                    withOffset(offset).
                    withPrefix(prefix).
                    withFilterPrefix(item.isStandard() ? filterStandardPrefix : filterPrefix);
